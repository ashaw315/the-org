...
- A dashboard at "/" already renders the product's self-referential metrics. The metric primitives live in lib/product-metrics.ts (north star, engagement, velocity, features shipped, sprints shipped, tests passing, NPS lift index). New features WIRE these primitives or add new views of them. Do NOT invent new analytics, telemetry, or instrumentation pipelines.
- Routes the engineering team must NEVER touch (they are the org's own documentation, rendered from the database): /changelog, /roadmap, /org, /status, /specs. Tickets never modify these.
